...
export function CaseCard({ c, priority }: { c: CaseDTO; priority?: boolean }) {
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card transition-all duration-200 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[var(--shadow-glow)]"
    >
      <div className="relative flex h-40 items-center justify-center overflow-hidden sm:h-44">
        ...
        <Image
          ...
          height={160}
          priority={priority}
          className="relative h-32 w-auto transition-transform duration-300 group-hover:-translate-y-1.5 group-hover:scale-105 sm:h-36"
        />
